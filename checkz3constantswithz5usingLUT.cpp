...
constexpr auto LUT = []
{
	...
	std::array<ResultT, (1'216'773)> arr = {};
...
int main(int argc, char *argv[])
{
	int estart = 0;
	int fstart = -10000;
	const long double lnpi = M_PIl;  //TODO: Note this is actually pi now, not ln(pi)!
	int prevlim = -1;
	int uplim5 = 1000;
	int uplim4 = 500;
	int uplim3 = 100;
	...
	int lowlim = -1 * uplim;
	const long double z5 = 1.036927755143369926331365486457034168L; //riemann_zetal((long double)5);
	const long double lnpi2 = powl(lnpi, (long double)2);
	const long double lnpi3 = powl(lnpi, (long double)3);
	const long double lnpi4 = powl(lnpi, (long double)4);
	const long double lnpi5 = powl(lnpi, (long double)5);
	const long double v0max = (long double)uplim;
	const long double v1max = (long double)uplim1 * lnpi + v0max;
	const long double v2max = (long double)uplim2 * lnpi2 + v1max;
	const long double v3max = (long double)uplim3 * lnpi3 + v2max;
	const long double v4max = (long double)uplim4 * lnpi4 + v3max;
	const long double v1BreakoutHigh = z5 + v0max;
	const long double v1BreakoutLow = z5 - v0max;
	const long double v2BreakoutHigh = z5 + v1max;
	const long double v2BreakoutLow = z5 - v1max;
	const long double v3BreakoutHigh = z5 + v2max;
	const long double v3BreakoutLow = z5 - v2max;
	const long double v4BreakoutHigh = z5 + v3max;
	const long double v4BreakoutLow = z5 - v3max;
	const long double v5BreakoutHigh = z5 + v4max;
	const long double v5BreakoutLow = z5 - v4max;
	
	long double val, v1, v2, v3, v4, v5, valDif;
	...
	switch (argc) {
		case 3:
		{
			...
			if (!(ss >> fstart)) {
				cerr << "Invalid value for fstart: " << argv[2] << endl;
				fstart = -10000;
			}
		}
		case 2:
		{
			...
			if (!(ss2 >> estart)) {
				cerr << "Invalid value for estart: " << argv[1] << endl;
				estart = 0;
			}
		}
	}
