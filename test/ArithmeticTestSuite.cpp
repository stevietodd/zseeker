...
TEST(ArithmeticTestSuite, DoubleBasicallyEqualMacroTest) {
	...
	ASSERT_FALSE(DOUBLE_BASICALLY_EQUAL(pi, 3.1415926)); // not precise enough
	ASSERT_FALSE(DOUBLE_BASICALLY_EQUAL(pi, 3.14159264)); // more precise but not close enough
	ASSERT_TRUE(DOUBLE_BASICALLY_EQUAL(pi, 3.14159265)); // more precise and close enough
	ASSERT_TRUE(DOUBLE_BASICALLY_EQUAL(pi, 3.141592653)); // even more precise (and accurate)
	ASSERT_TRUE(DOUBLE_BASICALLY_EQUAL(pi, 3.141592659)); // even more precise and technically wrong but within tolerance
}
...
TEST(ArithmeticTestSuite, DoubleOperationsTest) {
	...
	ASSERT_TRUE(DOUBLE_BASICALLY_EQUAL(pi2, pi2pow));
	ASSERT_TRUE(DOUBLE_BASICALLY_EQUAL(pi4, pi4pow));

	// assert we can calculate known zeta values (and our zeta constants are accurate...enough)
	ASSERT_TRUE(DOUBLE_BASICALLY_EQUAL((pi2 / 6), ZETA2));
	ASSERT_TRUE(DOUBLE_BASICALLY_EQUAL((pi4 / 90), ZETA4));
	...
	ASSERT_TRUE(DOUBLE_BASICALLY_EQUAL(random1, 97854.161586214948));
	ASSERT_TRUE(DOUBLE_BASICALLY_EQUAL(random2, -94702.091478218225));
}	