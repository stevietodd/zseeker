...
// "Top Six" variant: same launch geometry as TopFive — flatten (quint, x, y) on grid.x,
// quart/cubic on grid.y / grid.z. Single launch (no host batching). The z dimension is
// handled in-kernel only; we use #pragma unroll so the compiler can fully unroll small z-ranges.
__global__ static void compareToZeta5loopPositiveOnlyTopSix(
    int *out,
    ...
    // Same work as TopFive; unroll helps when zEnd - zStart + 1 is small (e.g. megaman hacks).
#pragma unroll 32
...
std::vector<int*>* GpuQuinticFirstCheckerPositiveOnlyTopSix::findHits(
    const double needle,
    const double theConst,
    const int degree,
    const float *coeffArray,
    ...
    dim3 gridsizes(
        ...
        (quintRange == 0) ? 0U : (unsigned long long)(quartRange + blocksizes.y - 1) / (unsigned long long)blocksizes.y,
        (unsigned long long)(cubicRange + blocksizes.z - 1) / (unsigned long long)blocksizes.z
    );
    cout << "GpuQuinticFirstCheckerPositiveOnlyTopSix: Grid sizes: " << gridsizes.x << "," << gridsizes.y << "," << gridsizes.z
         << " (ranges: quint=" << quintRange << ",quart=" << quartRange << ",cubic=" << cubicRange
         << ",x=" << xRange << ",y=" << yRange << ")" << endl;
    ...
    compareToZeta5loopPositiveOnlyTopSix<<<gridsizes, blocksizes>>>(
        d_out,
        theConst,
        needle,
        d_coeffArray,
    ...
    if (err != cudaSuccess) {
        cerr << "CUDA kernel launch error (TopSix): " << cudaGetErrorString(err) << endl;
    ...
    if (err != cudaSuccess) {
        cerr << "CUDA kernel execution error (TopSix): " << cudaGetErrorString(err) << endl;
        cudaEventDestroy(kernelStart); cudaEventDestroy(kernelStop);
        cudaFree(d_loopStartEnds); cudaFree(d_out); cudaFree(d_doubleCoeffArray); cudaFree(d_coeffArray); cudaFree(d_doubleHitCount); cudaFree(d_hitCount);
        delete[] out; floatHitCount = 0; return results;
    }
    ...
    cout << "Kernel execution time (topSix): " << kernelTimeMs << " ms" << endl;
