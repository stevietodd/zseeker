...
// "Top Four" variant:
// Promote the x loop into the launch by flattening (quint,x) into grid.x.
// Each thread owns (quintInd, quartInd, cubicInd, x), does the remaining y/z loops.
__global__ static void compareToZeta5loopPositiveOnlyTopFour(
    int *out,
    ...
    // Decode (quint,x) from flattened index.
    const long long quintXThreadIdx = combinedBaseIndex + (long long)blockIdx.x * (long long)blockDim.x + (long long)threadIdx.x;
    const long long combinedTotal = (long long)quintRange * (long long)xRange;
    if (quintXThreadIdx >= combinedTotal) return;

    const int quintOffset = (int)(quintXThreadIdx / (long long)xRange);
    const int xOffset = (int)(quintXThreadIdx - (long long)quintOffset * (long long)xRange);

    const int quintInd = quintStart + quintOffset;
    const int x = xStart + xOffset;

    // Decode quart/cubic from y/z.
    ...
    // Positive-only symmetry skips (same as the original positive-only kernel)
    ...
    // v3 breakout (depends only on quint/quart/cubic)
    ...
    for (int y = yStart; y <= yEnd; y++) {
        ...
        if (v1 < v1BreakoutLow || v1 > v1BreakoutHigh) continue;
...
std::vector<int*>* GpuQuinticFirstCheckerPositiveOnlyTopFour::findHits(
    const double needle,
    const double theConst,
    const int degree,
    const float *coeffArray,
    ...
    (void)v3max; // keep same structure; v3max isn't needed directly below
    ...
    const long long combinedTotal = (long long)quintRange * (long long)xRange;
    dim3 gridsizes(
        (unsigned long long)(combinedTotal + blocksizes.x - 1) / (unsigned long long)blocksizes.x,
        (quintRange == 0) ? 0 : (unsigned long long)(quartRange + blocksizes.y - 1) / (unsigned long long)blocksizes.y,
        (unsigned long long)(cubicRange + blocksizes.z - 1) / (unsigned long long)blocksizes.z
    );
    cout << "GpuQuinticFirstCheckerPositiveOnlyTopFour: Grid sizes: " << gridsizes.x << "," << gridsizes.y << "," << gridsizes.z
         << " (ranges: quint=" << quintRange << ",quart=" << quartRange << ",cubic=" << cubicRange << ",x=" << xRange << ")" << endl;
    ...
    compareToZeta5loopPositiveOnlyTopFour<<<gridsizes, blocksizes>>>(
        d_out,
        theConst,
        needle,
        d_coeffArray,
    ...
    cout << "Kernel execution time (topFour): " << kernelTimeMs << " ms" << endl;
